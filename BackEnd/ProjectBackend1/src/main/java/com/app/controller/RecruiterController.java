...
public class RecruiterController {
	...
	@PostMapping("/login")
	public ResponseEntity<Object> checkLogin(@Valid @RequestBody RecruiterLoginDTO recruiterDto,
	...
	try {
		Recruiter recruiter = recruiterService.checkLogin(recruiterDto);
		if (recruiter.getUserName().equals(recruiterDto.getUserName()) && recruiter.getPassword().equals(recruiterDto.getPassword())) {
			return new ResponseEntity<>(recruiter , HttpStatus.ACCEPTED);
		}
		else {
			return new ResponseEntity<>(recruiter, HttpStatus.ACCEPTED);
		}
	}catch(InvalidRecruiterException exception) {
        throw new InvalidRecruiterException("One or more enterd Field contain invalid objects");	
	}
	}
